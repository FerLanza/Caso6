...
var Bird = function(scene, octree, obstacles, velocity_hash, position, orientation,id) {
	...
	this.id = id;			//Id que se le asigna a cada ave 
	...
	this.distances = 0;			//Atributo que lleva la suma de las distancias de cada ave para pintar
	this.velocities = 0;		//Atributo que lleva la suma de las velocidades de cada ave para pintar

}

Bird.prototype.init_mesh = function(position, orientation) {
	var geometry = new BirdObject();
	var material =  new THREE.MeshBasicMaterial( { color: 0x828282, side: THREE.DoubleSide } ); //Cambio de color inicial a las aves a gris 
	//this.material = THREE.MeshBasicMaterial({color: 0x86666 , side: THREE.DoubleSide});\
	//material.color= 0x86666
...
Bird.prototype.update_position = function(elapsed_time, lookAt) {
	...
	//Aqui se deben pegar las variables que se imprimen en c++
		var maxDistanceOne = 1.92158;
		var maxDistanceTwo = 1.1355;
		var minDistanceOne = 0.149912;
		var minDistanceTwo = 0.165427;
		var maxVelocityOne = 6.41481;
		var maxVelocityTwo = 5.95577;
		var minVeolocityOne = -0.568098;
		var minVelocityTwo = -0.499715;
	...
	if (elapsed_time/1000 < .5) {
		...
		this.velocities += ((this.velocity.x + this.velocity.y + this.velocity.z)/3);  		//Atributo que lleva la suma de las velocidades de un ave
		this.distances += (Math.sqrt(Math.pow(displacement.x,2)+Math.pow(displacement.y,2)+Math.pow(displacement.z,2))); //atributo que lleva la suma de las distancias de un ave
	}
	if(elapsed_time/1000 < .10){
		if (this.distances > maxDistanceOne){			//Estos if son los condicionales para el pintado de las aves 
			this.mesh.material.Color = new THREE.Color( 0X336BFF );
		}
		else if (this.distances > maxDistanceTwo){
			this.mesh.material.color = new THREE.Color( 0xFF4933 );
		}
		if (this.distances < minDistanceOne){
			this.mesh.material.color = new THREE.Color( 0x237618 );
		}
		else if (this.distances < minDistanceTwo){
			this.mesh.material.color = new THREE.Color( 0xFFF74A );
		}
		if (this.velocities < maxVelocityOne){
			this.mesh.material.color = new THREE.Color( 0xA74AFF);
		}
		else if (this.velocities < maxVelocityTwo){
			this.mesh.material.color = new THREE.Color( 0xFF4AD9 );
		}
		if (this.velocities > minVeolocityOne){
			this.mesh.material.color = new THREE.Color( 0xA87959 );
		}
		else if (this.velocities > minVelocityTwo){
